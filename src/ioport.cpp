...
void RxThread::Run(pth_sem_t *stop1)
{
    ...
    while ((retval = port_m->get(buf, sizeof(buf), stop_m)) > 0)
    {
        ...
        {
            //            logger_m.debugStream() << "Calling onDataReceived on listener for " << port_m->getID() <<
            //            endlog;
...
{
    logger_m.infoStream() << "send(buf, len=" << len << "):" << buf << endlog;

    if (sockfd_m >= 0)
    {
        ssize_t nbytes = pth_sendto(sockfd_m, buf, len, 0, (const struct sockaddr *)&addr_m, sizeof(addr_m));
        if (nbytes == len)
        {
            return nbytes;
        }
...
{
    logger_m.debugStream() << "get(buf, len=" << len << "):" << buf << endlog;
    if (sockfd_m >= 0)
    {
        socklen_t rl;
        sockaddr_in r;
        rl = sizeof(r);
        memset(&r, 0, sizeof(r));
        ssize_t i = pth_recvfrom_ev(sockfd_m, buf, len, 0, (struct sockaddr *)&r, &rl, stop);
...
TcpClientIOPort::Socket::Socket(TcpClientIOPort *ioport) : ioport_m(ioport), sockfd_m(-1)
{
...
void TcpClientIOPort::importXml(ticpp::Element *pConfig)
{
    ...
    logger_m.infoStream() << "TcpClientIOPort " << (permanent_m ? "(permanent) " : "") << "configured for host "
                          << host_m << " and port " << port_m << endlog;
}
...
void RxCondition::onDataReceived(const uint8_t *buf, unsigned int len)
{
    ...
    if (cl_m)
    {
        if (regexFlag_m)
        {
            ...
            if (status == 0)
            {
                logger_m.debugStream() << "RxCondition: expected message received: '" << rx << "'" << regex_m.re_nsub
                                       << endlog;
                for (int i = 0; i < nmatch; i++)
                {
                    Object *obj = objects_m[i];
                    regmatch_t match = pmatch_m[i];
                    logger_m.debugStream()
                        << "subgroup: " << i << " '" << match.rm_so << ":" << match.rm_eo << "'" << endlog;
                    if ((match.rm_so != static_cast<size_t>(-1)) && (obj))
                    {
                        std::string newValue(rx.c_str(), match.rm_so, match.rm_eo - match.rm_so);
                        logger_m.debugStream() << "RxCondition: new value " << newValue << " found in regex for object "
                                               << obj->getID() << endlog;
                        ...
                        {
                            logger_m.errorStream() << "RxCondition: Error cannot set value '" << newValue
                                                   << "' to object '" << obj->getID() << "'" << endlog;
                        }
                    }
                }
                value_m = true;
