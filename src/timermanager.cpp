...
{
    if (!tryResolveUnprojected(current, from, to))
...
bool DateTime::tryResolveUnprojected(const DateTime &current, FieldType from, FieldType to)
{
    while (tryResolveWithoutWeekdays(current, from, to))
    {
        ...
        if (!this->tryIncreaseClosestGreaterFreeField(Day))
            return false;
    }
...
bool DateTime::tryResolveWithoutWeekdays(const DateTime &current, FieldType from, FieldType to)
{
    ...
    for (FieldType fieldId = Year; fieldId < from; fieldId = static_cast<FieldType>(fieldId + 1))
    {
        ...
        if (targetField > currentField)
            return true;
        if (targetField < currentField)
            return false; // Target is after current and those fields are not adjustable.
    }
...
TimeSpec::TimeSpec(int min, int hour, int mday, int mon, int year, int offset)
    : min_m(min), hour_m(hour), mday_m(mday), mon_m(mon), year_m(year), wdays_m(All), offset_m(offset),
...
void TimeSpec::checkIsValid() const
{
    ...
    {
        std::stringstream msg;
        msg << "Time spec is not valid: day=" << mday_m << " month=" << mon_m << " year=" << year_m
            << " hour=" << hour_m << " min=" << min_m << std::endl;
        throw ticpp::Exception(msg.str());
    }
}
...
bool TimeSpec::isValid() const
{
    ...
    if (hasDay)
    {
        ...
        if (mday_m > maxMonthDay)
            return false;
    }

    if (hour_m != -1 && (hour_m < 0 || hour_m > 23))
        return false;
    if (min_m != -1 && (min_m < 0 || min_m > 59))
        return false;
...
void TimeSpec::importXml(ticpp::Element *pConfig)
{
    ...
    infoStream("TimeSpec") << year_m + 1900 << "-" << mon_m + 1 << "-" << std::setfill('0') << std::setw(2) << mday_m
                           << " " << std::setfill('0') << std::setw(2) << hour_m << ":" << std::setfill('0')
                           << std::setw(2) << min_m << ":0 (wdays=" << wdays_m << "; exception=" << exception_m << ")"
                           << endlog;
}
...
void PeriodicTask::reschedule(time_t now)
{
    ...
    if (nextExecTime_m != 0)
    {
        ...
        logger_m.infoStream() << "Rescheduled at " << timeinfo.tm_year + 1900 << "-" << timeinfo.tm_mon + 1 << "-"
                              << timeinfo.tm_mday << " " << std::setfill('0') << std::setw(2) << timeinfo.tm_hour << ":"
                              << std::setfill('0') << std::setw(2) << timeinfo.tm_min << ":" << std::setfill('0')
                              << std::setw(2) << timeinfo.tm_sec << " (" << nextExecTime_m << ")" << endlog;
...
time_t PeriodicTask::findNext(time_t start, TimeSpec *next)
{
    ...
    {
        logger_m.errorStream() << "Timer error, nextExecTime(" << nextExecTime << ") is before startTime(" << start
                               << ")" << endlog;
        return 0;
    }
...
void PeriodicTask::statusXml(ticpp::Element *pStatus)
{
    ...
    memcpy(&timeinfo, localtime(&nextExecTime_m), sizeof(struct tm));
    execTime << timeinfo.tm_year + 1900 << "-" << timeinfo.tm_mon + 1 << "-" << timeinfo.tm_mday << " "
...
void FixedTimeTask::reschedule(time_t now)
{
    ...
    if (execTime_m > now)
    {
        ...
        logger_m.infoStream() << "Rescheduled at " << timeinfo.tm_year + 1900 << "-" << timeinfo.tm_mon + 1 << "-"
                              << timeinfo.tm_mday << " " << std::setfill('0') << std::setw(2) << timeinfo.tm_hour << ":"
                              << std::setfill('0') << std::setw(2) << timeinfo.tm_min << ":" << std::setfill('0')
                              << std::setw(2) << timeinfo.tm_sec << " (" << execTime_m << ")" << endlog;
        Services::instance()->getTimerManager()->addTask(this);
    }
    else
        logger_m.infoStream() << "Not rescheduled" << endlog;
...
void DaySpec::importXml(ticpp::Element *pConfig)
{
    ...
    debugStream("DaySpec") << year_m + 1900 << "-" << mon_m + 1 << "-" << mday_m << endlog;
}
...
bool ExceptionDays::isException(time_t time)
{
    ...
    for (it = daysList_m.begin(); it != daysList_m.end(); it++)
    {
        ...
        {
            infoStream("ExceptionDays") << timeinfo.tm_year + 1900 << "-" << timeinfo.tm_mon + 1 << "-"
