            ...
            else
            {
                ...
                if (output_m.length())
                {
                    ...
                    if (maxSize_m > 0)
                    {
                        ...
                        app = new log4cpp::RollingFileAppender("RollingFileAppender", output_m, maxSize_m * 1024,
                                                               maxIndex_m);
                    }
                    else
...
{
    return Logger::getInstance(cat).errorStream();
};
WarnStream warnStream(const char *cat)
{
    return Logger::getInstance(cat).warnStream();
};
LogStream infoStream(const char *cat)
{
    return Logger::getInstance(cat).infoStream();
};
DbgStream debugStream(const char *cat)
{
    return Logger::getInstance(cat).debugStream();
};
bool isDebugEnabled(const char *cat)
{
    return Logger::getInstance(cat).isDebugEnabled();
};
