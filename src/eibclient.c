...
/** resolve host name */
static int GetHostIP(struct sockaddr_in *sock, const char *Name)
{
...
static int SendRequest(EIBConnection *con, unsigned int size, uchar *data)
{
    uchar head[2];
    int i, start;
...
static int CheckRequest(EIBConnection *con)
{
    int i;
    if (con->readlen < 2) {
        uchar head[2];
    ...
    if (con->readlen < con->size + 2) {
        i = pth_read_ev(con->fd, con->buf + (con->readlen - 2), con->size - (con->readlen - 2), con->ev);
        if (i == -1 && errno == EINTR)
            return 0;
        if (i == -1)
            return -1;
...
static int GetRequest(EIBConnection *con)
{
    do {
        if (CheckRequest(con) == -1)
            return -1;
    } while (con->readlen < 2 || (con->readlen >= 2 && con->readlen < con->size + 2));
...
static int OpenBusmonitor_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
static int OpenBusmonitorText_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
static int OpenVBusmonitor_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
static int OpenVBusmonitorText_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
static int OpenT_Connection_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
static int OpenT_TPDU_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
static int OpenT_Individual_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
int EIBOpenT_Individual_async(EIBConnection *con, eibaddr_t dest, int write_only)
{
    uchar head[5];
    int i;
    if (!con) {
...
static int OpenT_Group_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
int EIBOpenT_Group_async(EIBConnection *con, eibaddr_t dest, int write_only)
{
    uchar head[5];
    int i;
    if (!con) {
...
static int OpenT_Broadcast_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
int EIBSendTPDU(EIBConnection *con, eibaddr_t dest, int len, uint8_t *data)
{
    uchar *ibuf;
    int i;
    if (!con) {
...
int EIBGetAPDU_Src(EIBConnection *con, int maxlen, uint8_t *buf, eibaddr_t *src)
{
    int i;
    if (!con || !buf) {
        errno = EINVAL;
...
static int Open_GroupSocket_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
int EIBSendGroup(EIBConnection *con, eibaddr_t dest, int len, uint8_t *data)
{
    uchar *ibuf;
    int i;
    if (!con) {
...
static int M_ReadIndividualAddresses_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
int EIB_M_ReadIndividualAddresses_async(EIBConnection *con, int maxlen, uint8_t *buf)
{
    uchar head[2];
    if (!con) {
        errno = EINVAL;
...
int EIB_M_ReadIndividualAddresses(EIBConnection *con, int maxlen, uint8_t *buf)
{
...
static int M_Progmode_On_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
static int M_Progmode_Off_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
static int M_Progmode_Toggle_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
static int M_Progmode_Status_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
static int M_GetMaskVersion_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
static int M_WriteIndividualAddress_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
static int MC_Connect_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
static int MC_Progmode_On_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
static int MC_Progmode_Off_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
static int MC_Progmode_Toggle_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
static int MC_Progmode_Status_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
static int MC_GetMaskVersion_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
static int MC_GetPEIType_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
static int MC_ReadADC_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
int EIB_MC_ReadADC_async(EIBConnection *con, uint8_t channel, uint8_t count, int16_t *val)
{
    uchar head[4];
    if (!con) {
        errno = EINVAL;
...
int EIB_MC_ReadADC(EIBConnection *con, uint8_t channel, uint8_t count, int16_t *val)
{
...
static int MC_PropertyRead_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
int EIB_MC_PropertyRead_async(EIBConnection *con, uint8_t obj, uint8_t property, uint16_t start, uint8_t nr_of_elem,
...
int EIB_MC_PropertyRead(EIBConnection *con, uint8_t obj, uint8_t property, uint16_t start, uint8_t nr_of_elem,
    int max_len, uint8_t *buf)
{
    if (EIB_MC_PropertyRead_async(con, obj, property, start, nr_of_elem, max_len, buf) == -1)
        return -1;
    return EIBComplete(con);
}

static int MC_Read_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
int EIB_MC_Read_async(EIBConnection *con, uint16_t addr, int len, uint8_t *buf)
{
    uchar head[6];
    if (!con) {
        errno = EINVAL;
...
static int MC_PropertyWrite_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
int EIB_MC_PropertyWrite_async(EIBConnection *con, uint8_t obj, uint8_t property, uint16_t start, uint8_t nr_of_elem,
    int len, const uint8_t *buf, int max_len, uint8_t *res)
{
    uchar *ibuf;
    int i;
    if (!con) {
...
int EIB_MC_PropertyWrite(EIBConnection *con, uint8_t obj, uint8_t property, uint16_t start, uint8_t nr_of_elem, int len,
    const uint8_t *buf, int max_len, uint8_t *res)
{
    if (EIB_MC_PropertyWrite_async(con, obj, property, start, nr_of_elem, len, buf, max_len, res) == -1)
        return -1;
    return EIBComplete(con);
}
static int MC_Write_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
int EIB_MC_Write_async(EIBConnection *con, uint16_t addr, int len, const uint8_t *buf)
{
    uchar *ibuf;
    int i;
    if (!con) {
...
int EIB_MC_Write(EIBConnection *con, uint16_t addr, int len, const uint8_t *buf)
{
...
static int MC_PropertyDesc_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
int EIB_MC_PropertyDesc_async(EIBConnection *con, uint8_t obj, uint8_t property, uint8_t *type,
...
int EIB_MC_PropertyDesc(EIBConnection *con, uint8_t obj, uint8_t property, uint8_t *type, uint16_t *max_nr_of_elem,
    uint8_t *access)
{
    if (EIB_MC_PropertyDesc_async(con, obj, property, type, max_nr_of_elem, access) == -1)
        return -1;
    return EIBComplete(con);
}

static int MC_Authorize_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
static int MC_SetKey_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
static int MC_PropertyScan_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
...
static int LoadImage_complete(EIBConnection *con)
{
    int i;
    i = GetRequest(con);
    if (i == -1)
